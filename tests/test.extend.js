...
module('follow.extend.js');
	...
	test('model.sizeof([chain])', function()
	{
		...
		ok(
			model.sizeof({x: 1, y: 2}) == 2,
			'В качестве первого аргумента можно указать произвольный объект, не имеющий отношения к модели'
		);
		
		ok(
			model.sizeof(true) == 3 + data.length &&
			model.sizeof({x: 1, y: {y1: 2, y2: [1,2]}}, true) == 6
			,
			'В качестве второго аргумента можно указать true, тогда подсчет будет включать внутренние свойства объектов и массивов'
		);
	});
	...
	test('model.map([chain], [callback], [filter])', function()
	{
		...
		ok(
			model.map([3,2,1], function(v){ return v }).reverse().join('-') == '1-2-3',
			'В качестве первого аргумента chain может выступать произвольный объект, не имеющий отношений к модели'
		);
	});
