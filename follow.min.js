 ...
 */
Follow.utils.json_to_xml=function(d,c){if(typeof d=="string"){try{d=JSON.parse(d)}catch(h){d=Function("return "+d)()}}var a=this,b={str:['<model type="'+this.type(d)+'">',"","\n</model>"],prop:{open:function(e,j,i,k){return'<p name="'+e+'" type="'+j+'" path="'+i+'"'+k+">"},close:function(e){while(--e>1){b.str[1]+=this.offset(e)+"</p>"}},offset:function(e){return"\n"+Array(e).join("\t")}},doc:null},g={level:/\./g,quote:/"/g};var f=1;this.extend(true,{},d,function(j,l,i){var k=a.type(l),n=(j.match(g.level)||[]).length+2,m=b.prop.offset(n),e=["array","object"].indexOf(k)!==-1,l=!e?' value="'+String(l).replace(g.quote,'\\"')+'"/':"";f>n&&b.prop.close(f);f=n;b.str[1]+=m+b.prop.open(i,k,j,l)});f>1&&b.prop.close(f);b.str=b.str.join("");return(c?b.str:this.parse_xml(b.str))};Follow.utils.parse_xml=function(f){if(typeof DOMParser!="undefined"){return(new DOMParser()).parseFromString(f,"text/xml")}else{if(document.implementation||typeof ActiveXObject!="undefined"){var d;try{d=document.implementation.createDocument()}catch(c){d=new ActiveXObject("MSXML2.DOMDocument")}d.loadXML(f);return d}else{var a="data:text/xml;charset=utf8,"+encodeURIComponent(f);var b=new XMLHttpRequest();b.open("GET",a,false);b.send(null);return b.responseXML}}};
/*!
 * Follow.js
 * Helper to easy find and update data in the model
 */
Follow.extend({select:function(c,f){var d=this,b=[],g=Follow.utils.json_to_xml(d());if(f&&g.querySelectorAll){b=[].slice.call(g.querySelectorAll(f))}else{if(document.evaluate){var a=g.evaluate(c,g,null,XPathResult.ANY_TYPE,null);while(node=a.iterateNext()){b.push(node)}}else{var a=g.documentElement.selectNodes(c);for(var e=0;e<a.length;e++){b.push(a[e])}}}this.extend(b,{update:function(h){this.forEach(function(j){var i=j.getAttribute("path");d(i,h)})},remove:function(){this.update(undefined)}});return b}});