 ...
 * Core of the library.
 */
(function(d){var b={},a={};function c(k,o,l){var o=o||b,k=k||"default",m=o[k],j=a[k];l&&(o[k]="");if(j&&o==j.storage){return j.model}var n=function(p,y,u){var z=n,v=e(arguments),t=JSON.parse(z.toJSON()),p=p instanceof Array?p.join("."):p,A=typeof p=="object"?{}:String(p).split("."),q,x=t,u={if_not_defined:u=="safe",is_forced:u=="force"};function w(){while(q=A.shift()){if(q&&x[q]!==d){x=x[q]}else{return null}}return x}function r(){while(q=A.shift()){if(A.length==0){var D=x[q],F=D===d,E={chain:p,prop:q,parent:x},B=typeof y=="function"?y.apply(z,[typeof D!="object"?D:z.clone(D),f({value:D},E)]):y,C=B===d;(!u.if_not_defined||F)&&((C&&!F)||!C)&&((z.toJSON(p)!==i(B))||(B===null&&F)||(u.is_forced))&&function(){(B===d&&x instanceof Array)?x.splice(q,1):(x[q]=B);o[k]=i(t);z.tracking(p);z.broadcast(p,[B,f({value:{prev:D,current:B}},E)])}()}else{!x[q]&&(x[q]={});x=x[q]}}}z.__hook&&z.__hook.apply(z,v);if(typeof p=="object"){for(var s in p){p.hasOwnProperty(s)&&z(s,p[s],v[1])}return z}else{if(v.length==1){return z.__get(w(),v)}else{if(v.length>=2){r();return z}}}return t};f(n,{constructor:c,modelName:k,storage:o,followers:{},dependency:{composite:{},on:{}},backups:[],wrappers:[],__hook:null,__get:function(p){return p},extend:f,serialize:i,clone:g,init:function(p){this(m?JSON.parse(m):p||{});return this},toString:function(p){return p?i(this(p)):o[k]||"{}"}},c.prototype);a[k]={model:n,storage:o};return n}function i(j){return JSON.stringify(j,null,"\t")}function e(j){return[].slice.call(j)}function g(j){return JSON.parse(JSON.stringify(j))}function h(j){return Object.prototype.toString.call(j)}function f(j,o){var k=e(arguments),n=j===true&&k.shift(),o=k.shift(),m=function(){return k[k.length-1]},p,l;typeof m()=="string"&&(l=k.pop());typeof m()=="function"&&(p=k.pop());k.forEach(function(w){for(var r in w){if(w.hasOwnProperty(r)){var v=o[r],t=w[r],q=v==null,x=!h(t).match(/\[object (Array|Object)\]/),s=[true,o[r],t];if(p){var u=l?l+"."+r:String(r);p(u);s.push(p,u)}if(x||!n){o[r]=t}else{if(n){if(q||typeof v!="object"){s[1]=t instanceof Array?[]:{};o[r]=f.apply(this,s)}else{f.apply(this,s)}}}}}});return o}c.extend=function(){f.apply(this,[true].concat(this.prototype,e(arguments)))};this.Follow=c}());
/*!
 * Follow.js
