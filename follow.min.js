 ...
 */
Follow.extend({select:function(g,d){var e=this,b=[],h=Follow.utils.json_to_xml(e()),c=h.documentElement;if(g&&h.querySelectorAll){b=[].slice.call(c.querySelectorAll(g))}else{if(d){if(document.evaluate){var a=h.evaluate(d,c,null,XPathResult.ANY_TYPE,null);while(node=a.iterateNext()){b.push(node)}}else{var a=c.selectNodes(d);for(var f=0;f<a.length;f++){b.push(a[f])}}}}this.extend(b,{update:function(i){this.forEach(function(k){var j=k.getAttribute("path");e(j,i)})},remove:function(){this.update(undefined)}});return b}});