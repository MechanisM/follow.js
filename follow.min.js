 ...
 */
(function(b){function a(c,d){var c=[].concat(c);if(b.yepnope){yepnope({load:c,complete:d})}else{if(b.jQuery){c=c.map(function(e){return $.ajax({url:e,dataType:"script"})});$.when.apply(this,c).then(d)}else{c.forEach(function(e){document.write('<script type="text/javascript" src="'+e+'"><\/script>')});d()}}}Follow.load=function(e,d){var c=[];e.forEach(function(h){var f=Follow(h.name,h.storage),g=h.module;if(g){this.modules=this.modules||{};this.modules[g.name]=g;g._model=f;g._params=d;c.push(g)}h.chains.forEach(function(i){f(i.chain,i.json)})},this);c.forEach(function(h){var g=function(){var i=(h.path||h._params.modules)+h.name+".js";a(i)};if(h.depends){var f=h._model;h.depends.trim().split(" ").every(function(i){var k=f(i),j=f.gettype(k);return(j=="array"||j=="object"?f.sizeof(k):j=="number"?String(k):k)})&&g()}else{g()}})};Follow.module=function(e){var f=this.modules[e.name],c=e.depends,h=function(){if(e.init(f._model,f._params)===e){this.modules[e.name]._inited=true}}.bind(this);if(c){if(c.modules){var d=c.modules.trim().split(" ").filter(function(i){return !(this.modules[i]&&this.modules[i]._inited)},this);if(d.length){b.console&&console.error('Module "'+f.name+'" initialization error: module(s) "'+d.join(", ")+'" was not loaded or their init-method not returns this-object.');return false}}if(c.external){var g=c.external.trim().split(" ").map(function(i){return f._params.external+i+".js"});a(g,h)}else{h()}}else{h()}}}(this));