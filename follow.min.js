 ...
 */
(function(c,b){function a(d,e){var d=[].concat(d);if(b.yepnope){yepnope({load:d,complete:e})}else{if(b.jQuery){c.when.apply(this,d.map(function(f){return c.ajax({url:f,dataType:"script"})})).then(e)}else{d.forEach(function(f){document.write('<script type="text/javascript" src="'+f+'"><\/script>')});e()}}}Follow.load=function(f,e){var d=[];f.forEach(function(h){var g=Follow(h.name,h.storage);h.chains.forEach(function(l){var k=l.json,j=l.module,i=d.filter(function(m){return m.chain==l.chain&&m.model==g}).shift();if(i){j&&!i.module&&(i.module=j);i.json=i.json instanceof Array?i.json.concat(k):k}else{d.push({model:g,chain:l.chain,module:j,external:h.dependency,json:k})}})});d.forEach(function(i){var g=function(){var j=i.model;i.module?a(e.modules+i.module+".js",function(){j(i.chain,i.json)}):j(i.chain,i.json)};if(i.external){var h=i.external.split(" ").map(function(j){return e.external+j+".js"});a(h,g)}else{g()}})};Follow.module=function(d){Follow(d.model,d.storage).follow(d.chain,d.init,"once")}}(window.jQuery,window));