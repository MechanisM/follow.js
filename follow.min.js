 ...
 */
(function(b){function a(c,d){var c=[].concat(c);if(b.yepnope){yepnope({load:c,complete:d})}else{if(b.jQuery){c=c.map(function(e){return $.ajax({url:e,dataType:"script"})});$.when.apply(this,c).then(d)}else{c.forEach(function(e){document.write('<script type="text/javascript" src="'+e+'"><\/script>')});d()}}}Follow.load=function(d,c){d.forEach(function(j){var f=Follow(j.name,j.storage),h=j.dependency,e=j.isModule;Follow.module.params=Follow.module.params||{};Follow.module.params[f.modelName]=c;j.chains.forEach(function(k){f(k.chain,k.json)});if(e){var g=function(){a(c.modules+f.modelName+".js")};if(h){var i=h.split(" ").map(function(k){return c.external+k+".js"});a(i,g)}else{g()}}})};Follow.module=function(d){var c=Follow(d.model,d.storage),e=this.module.params[c.modelName];d.init(c,e)}}(this));