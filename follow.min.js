 ...
 */
(function(d,c){var b;function a(e,f){var e=[].concat(e);if(c.yepnope){yepnope({load:e,complete:f})}else{if(c.jQuery){e=e.map(function(g){return d.ajax({url:g,dataType:"script"})});d.when.apply(this,e).then(f)}else{e.forEach(function(g){document.write('<script type="text/javascript" src="'+g+'"><\/script>')});f()}}}Follow.load=function(g,f){var e=[];b=f;g.forEach(function(i){var h=Follow(i.name,i.storage);i.chains.forEach(function(m){var l=m.json,k=m.module,j=e.filter(function(n){return n.chain==m.chain&&n.model==h}).shift();if(j){k&&!j.module&&(j.module=k);j.json=j.json instanceof Array?j.json.concat(l):l}else{e.push({model:h,chain:m.chain,module:k,external:i.dependency,json:l})}})});e.forEach(function(j){var h=function(){var k=j.model;j.module?a(f.modules+j.module+".js",function(){k(j.chain,j.json)}):k(j.chain,j.json)};if(j.external){var i=j.external.split(" ").map(function(k){return f.external+k+".js"});a(i,h)}else{h()}})};Follow.module=function(e){this(e.model,e.storage).follow(e.chain,function(){var f=[].slice.call(arguments).concat(b);e.init.apply(this,f)},"once")}}(window.jQuery,window));