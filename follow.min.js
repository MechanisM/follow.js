 ...
 */
(function(a){Follow.load=function(d,b){var c=[];d.forEach(function(f){var e=Follow(f.name,f.storage);f.chains.forEach(function(j){var i=j.json,h=j.module,g=c.filter(function(k){return k.chain==j.chain&&k.model==e}).shift();if(g){h&&!g.module&&(g.module=h);g.json=g.json instanceof Array?g.json.concat(i):i}else{c.push({model:e,chain:j.chain,module:h,json:i})}})});c.forEach(function(f){var e=f.model;if(f.module){a.ajax({url:b+f.module+".js",dataType:"script",complete:function(){e(f.chain,f.json)}})}else{e(f.chain,f.json)}})};Follow.module=function(b){Follow(b.model,b.storage).follow(b.chain,b.init,"once")}}(window.jQuery));