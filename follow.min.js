 ...
 * Dependencies: jQuery
 */
this.jQuery&&(function(b){var a={links:[]};Follow.link=function(c){a.links.push(c);return a.links};b(document).delegate("input[data-follow], select[data-follow], textarea[data-follow]","change",function(){var d=b(this),e={chain:d.data("follow"),model:d.data("follow-model"),storage:d.data("follow-storage")},c=Follow(e.model,e.storage);if(d.is(":checkbox")){c(e.chain,d.prop("checked"))}else{c(e.chain,d.val())}});Follow.sync=function(d,f){var c=this,e=f[0];b('*[data-follow="'+d+'"]').each(function(){var g=b(this),h=false;b.each(a.links,function(){return(((this.model&&this.model===c)||true)&&g.closest(this.location).length&&(h=true)&&this.trigger(g,e))});if(!h){if(g.is(":checkbox")||g.is(":radio")){(g.is(":checkbox")||(g.is(":radio")&&g.val()==e))&&g.attr("checked",!!e)}else{if(g.is("input")||g.is("select")||g.is("textarea")){g.is("select[multiple]")&&c.gettype(e)!="array"&&(e=c.map(e,function(i){return i}));g.val(e)}else{if(g.is("img")){g.attr("src",e)}else{g.html(e)}}}}})};Follow.extend({importDataFromDOM:function(){var c=this,d={};b("[data-follow]").each(function(){var f=b(this),e=f.data("follow"),h={model:f.data("follow-model"),storage:f.data("follow-storage")},i=Follow(h.model,h.storage);if(c===i&&!d[e]){var g;if(f.is(":checkbox")){g=f.prop("checked")}else{if(f.is(":radio")&&f.attr("name")){g=b(':radio[name = "'+f.attr("name")+'"]').val()}else{if(f.is("input")||f.is("select")||f.is("textarea")){g=f.val()}else{g=f.text()}}}d[e]=true;c(e,g)}})}})}(this.jQuery));
/*!
