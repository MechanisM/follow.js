 ...
 */
(function(a){Follow.load=function(e,b){var d={};e.forEach(function(g){var f=Follow(g.name,g.storage);g.chains.forEach(function(i){var h=d[i.chain]||(d[i.chain]={model:f,chain:i.chain,module:i.module});h.json=h.json instanceof Array?h.json.concat(i.json):i.json})});for(var c in d){d.hasOwnProperty(c)&&function(g){var f=g.model;if(g.module){a.ajax({url:b+g.module+".js",dataType:"script",complete:function(){f(g.chain,g.json)}})}else{f(g.chain,g.json)}}(d[c])}};Follow.module=function(b){Follow(b.model,b.storage).follow(b.chain,b.init,"once")}}(window.jQuery));