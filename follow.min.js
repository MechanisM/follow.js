 ...
 * Core of the library.
 */
(function(){var e={};function b(f,j,g){var j=j||e,f=f||"default",h=j[f];g&&(j[f]="");var i=function(k,t,p){var u=i,q=d(arguments),o=JSON.parse(u.toJSON()),k=k instanceof Array?k.join("."):k,v=typeof k=="object"?{}:String(k).split("."),l,s=o,p={if_not_defined:p=="safe",is_forced:p=="force"};function r(){while(l=v.shift()){if(l&&s[l]!==undefined){s=s[l]}else{return null}}return s}function m(){while(l=v.shift()){if(v.length==0){var x=s[l],z=x===undefined,y={chain:k,prop:l,parent:s},w=typeof t=="function"?t.apply(u,[typeof x!="object"?x:u.clone(x),c({value:x},y)]):t;((u.toJSON(k)!==a(w))||(w===null&&z)||(p.is_forced))&&(!p.if_not_defined||z)&&function(){(w===undefined&&s instanceof Array)?s.splice(l,1):(s[l]=w);j[f]=a(o);u.tracking(k);u.broadcast(k,[w,c({value:{prev:x,current:w}},y)])}()}else{!s[l]&&(s[l]={});s=s[l]}}}u.__hook&&u.__hook.apply(u,q);if(typeof k=="object"){for(var n in k){k.hasOwnProperty(n)&&u(n,k[n],q[1])}return u}else{if(q.length==1){return u.__get(r(),q)}else{if(q.length>=2){m();return u}}}return o};c(i,{constructor:b,modelName:f,storage:j,followers:{},dependency:{composite:{},on:{}},backups:[],wrappers:[],__hook:null,__get:function(k){return k},extend:c,clone:function(k){return JSON.parse(JSON.stringify(k))},init:function(k){this(h?JSON.parse(h):k||{});return this},toString:function(k){return k?a(this(k)):j[f]||"{}"}},b.prototype);return i}function a(f){return JSON.stringify(f,null,"\t")}function d(f){return[].slice.call(f)}function c(f,k){var g=d(arguments),j=f===true&&g.shift(),k=g.shift(),i=function(){return g[g.length-1]},l,h;typeof i()=="string"&&(h=g.pop());typeof i()=="function"&&(l=g.pop());g.forEach(function(p){for(var m in p){if(p.hasOwnProperty(m)){var o=null,r=p[m];j&&typeof r=="object"&&r!==null&&(o=r.constructor==Array?[]:r.constructor==Object?{}:"");var n,q=[true,o,k[m],r];l&&(n=h?h+"."+m:String(m),l(n),q.push(l,n));k[m]=o?c.apply(this,q):r}}});return k}b.extend=function(){c.apply(null,[true].concat(this.prototype,d(arguments)))};window.Follow=b}());
/*!
 * Follow.js
