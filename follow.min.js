 ...
 */
Follow.utils.json_to_xml=function(d,c){if(typeof d=="string"){try{d=JSON.parse(d)}catch(h){d=Function("return "+d)()}}var a=this,b={str:['<model type="'+this.type(d)+'">',"","\n</model>"],prop:{open:function(e,j,i,k){return'<p name="'+e+'" type="'+j+'" path="'+i+'"'+k+">"},close:function(e){while(--e>1){b.str[1]+=this.offset(e)+"</p>"}},offset:function(e){return"\n"+Array(e).join("\t")}},doc:null},g={level:/\./g,quote:/"/g};var f=1;this.extend(true,{},d,function(e,q,k){var p=a.type(q),j=(e.match(g.level)||[]).length+2,l=b.prop.offset(j),o=["array","object"].indexOf(p)!==-1;if(o){var n=true;for(var m in q){if(q.hasOwnProperty(m)){n=false;break}}q=n?"/":""}else{q=' value="'+String(q).replace(g.quote,"&quot;")+'"/'}f>j&&b.prop.close(f);f=j;b.str[1]+=l+b.prop.open(k,p,e,q)});f>1&&b.prop.close(f);b.str=b.str.join("");return(c?b.str:this.parse_xml(b.str))};Follow.utils.parse_xml=function(f){if(typeof DOMParser!="undefined"){return(new DOMParser()).parseFromString(f,"text/xml")}else{if(document.implementation||typeof ActiveXObject!="undefined"){var d;try{d=document.implementation.createDocument()}catch(c){d=new ActiveXObject("MSXML2.DOMDocument")}d.loadXML(f);return d}else{var a="data:text/xml;charset=utf8,"+encodeURIComponent(f);var b=new XMLHttpRequest();b.open("GET",a,false);b.send(null);return b.responseXML}}};
/*!
 * Follow.js
 * Helper to easy find and update data in the model
 */
Follow.extend({select:function(g,d){var e=this,b=[],h=Follow.utils.json_to_xml(e()),c=h.documentElement;if(g&&h.querySelectorAll){b=[].slice.call(c.querySelectorAll(g))}else{if(d){if(document.evaluate){var a=h.evaluate(d,c,null,XPathResult.ANY_TYPE,null);while(node=a.iterateNext()){b.push(node)}}else{var a=c.selectNodes(d);for(var f=0;f<a.length;f++){b.push(a[f])}}}}b=b.map(function(i){return{name:i.getAttribute("name"),type:i.getAttribute("type"),path:i.getAttribute("path"),value:i.getAttribute("value")}});this.extend(b,{update:function(i){this.forEach(function(j){e(j.path,i)})},remove:function(){this.update(undefined)}});return b}});