 ...
 * Dependencies: jQuery
 */
this.jQuery&&(function(b){var a={links:[]};Follow.sync=function(d,f){var c=this,e=f[0];b('*[data-follow="'+d+'"]').each(function(){var g=b(this),h=false;b.each(a.links,function(){return(((this.model&&this.model===c)||true)&&g.closest(this.location).length&&(h=true)&&this.trigger(g,e))});if(!h){if(g.is(":checkbox")||g.is(":radio")){g.attr("checked",!!e)}else{if(g.is("input")||g.is("select")||g.is("textarea")){g.is("select[multiple]")&&c.gettype(e)!="array"&&(e=c.map(e,function(i){return i}));g.val(e)}else{if(g.is("img")){g.attr("src",e)}else{g.html(e)}}}}})};Follow.link=function(c){a.links.push(c);return a.links}}(this.jQuery));
/*!
